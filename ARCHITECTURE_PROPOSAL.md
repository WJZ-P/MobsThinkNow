...
│     ├─ SquadSocialChoreography          阶段/相位/职位到社交动作节拍的纯函数
...
- `BRIEFING`：成员转头看向首领，并在 24 tick 内完成巡视、商议、左右翼命令、点头/摇头和最终回应；
...
- `SquadSocialChoreography` 是不访问世界和实体的纯节拍层：`FORMING` 召集、长集结补召、
  24 tick 会议循环、换届重整和部署回应都只由 `state + squadId + phase + followerRoles` 决定。
  `squadId` 只用于稳定轮换发言成员，因此同一小队重载后不会突然换人，多支小队又不会动作完全同步；
- 会议循环依次发布首领巡视、成员商议、左翼命令、左翼点头、另一成员摇头、右翼修正命令和右翼确认；
  缺少对应侧翼时自动退化为通用指挥。声音只落在召集、命令、异议和最终回应，纯观察/商议/点头
  使用少量粒子，避免 20 只小队形成连续噪声；
...
- JUnit：效用选择、配置边界、六类总攻方案门槛、交叉射界/侧后爆破纯几何、苦力怕难度智力区间、观察点积、稳定绕后点、起爆/退火边界与移动引信速度硬上限、
  ...
  24 tick 会议顺序/缺席职位降级/部署错峰回应，以及招手、巡视、左右指令、点头、摇头、商议和
  前指总攻关键帧的轴向、镜像与收势、
...
- Minecraft 服务端 GameTest：生产 Mixin 安装、智力经过真实实体存读链保持不变、
  ...
  验证召集—巡视—左右翼命令—点头/摇头—回应完整会议序列、部署总攻与错峰确认，并校验首领/
  成员成功动作指标；受击撤退触发与 5 格安全半径终止、
  ...
  额外乘员；阵型间距按本批最大真实碰撞宽度自适应，末影人另覆盖生产 Mixin Goal/职业分配
   ...
   六个复数分类、`nether`/`overworld_assault` 分类与五十七个战术/变种 literal 全量注册，并真实批量生成十四类基础实体；当前共 145 项
  服务端 GameTest；
