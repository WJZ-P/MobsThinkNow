...
- 共享 `ShieldCombatPlanner` 同时服务 Fabric 与 Paper，统一随机守候/反击延迟、信号寿命和水平正面夹角。
  ...
  `EntityDamageByEntityEvent` 中只补齐成熟举盾、正面近战/扫击/投射物的结算；伤害事件直接从实体
  yaw/pitch 计算水平朝向，不创建 EyeLocation/Vector；背刺和其他伤害照常命中；
...
- Fabric/Paper 现在共用纯 Java `CoverPositionPlanner`：它只处理固定半径的格点顺序、有效射程、稳定侧向
  打散与评分；是否可站、是否被遮挡、探头处是否有射界由平台主线程适配器通过 primitive XYZ 提供，
  搜索只为最终入选计划物化 `GridPosition`，也不把 Bukkit 世界对象放进共享层；
- Paper 默认只让 IQ 至少 5 的持弓骷髅使用该战术。一次搜索最多检查半径四格内 96 个藏身候选，最多
  返回 4 个方案；藏身格和探头格必须同时位于当前 IQ 的紧急脱离触发圈之外，再依次交给公开
  `Pathfinder`，所以方块查询和 A* 次数都有硬上限，也不会选中立即被高优先级脱离 Goal 抢占的近点；
  失败后默认等待 60 tick；
...
- IQ 至少 7 且副手有真实烟花火箭时，弩手会在 6～30 格射程与爆心友军检查均通过后装入烟花；释放后
  ...
  48；受管弹体同时保存在 UUID 映射和全局双向链中，tick、定向清理与停用直接沿节点扫描。命中实体/
  方块后触发原版烟花爆炸（不破坏地形），40 tick 未命中也会空爆并回收；超容量、队友
...
- 接敌 Goal 根据 IQ 在直追、速度拦截和稳定左右侧翼之间切换；目标正看向苦力怕或举盾时，高 IQ 个体
  优先侧后切入，寻路失败按“侧翼 → 拦截 → 直追”逐级降级；注视、目标提前量与侧翼点直接读取实体
  标量快照，不创建 Bukkit Location/Direction 或共享输入向量；
- 点火 Goal 先向空间预约板申请预计爆点和预计爆炸 tick，成功后播放嘶声并继续追踪目标的速度提前量；
- `CreeperFeintPlanner` 已进入无平台依赖的共享层。IQ 8～10、未带电且引信尚未推进的个体在真实起爆圈外
  被目标注视或举盾时，以优先级 0 执行 `6～8 tick` 可见假点燃，再退火并从坐标、水平速度与朝向标量
  快照直接计算 9 格侧后退出线。
...
- 最多 64 项的退火所有权队列与最近 600 tick、最多 256 个佯爆完成标记都由 UUID 映射和双向链保存；
  退火任务直接扫描节点，重复完成会移到链尾，完成标记只从表头 O(1) 淘汰过期或超容量节点，不再每 tick
  构造集合迭代器；
...
- 预约板按世界与水平网格分桶，每次只扫描中心周围 `3x3` 桶，并受 `maximum-checks` 硬上限约束。
  预约中心直接保存 primitive XZ，世界内桶坐标压为 primitive long，Reservation 自身作为双向桶链节点；
  查询不构造 CellKey、Set 迭代器或二次 owner 查表。冲突距离以 primitive XZ 比较；达到上限时保守视作已占用，因此
  不存在“每只苦力怕扫描全服每只苦力怕”的平方退化；
...
- IQ 至少 6 的蜘蛛发现目标正观察自己或玩家正在举盾时，从稳定侧后接近；IQ 至少 5 的个体命中后
  短暂退到侧后方，再进入下一轮攻击。注视点积、截击、侧袭、跳扑与载具速度在共享层直接以水平
  标量计算，平台层只在最终 Pathfinder 边界构造一个位置；
...
- 服务同时维护 owner→block 与 world/chunk→block 反向索引；蜘蛛或区块卸载时只访问对应的少量键并
  立即恢复其陷阱；世界卸载、mobGriefing 热关闭、容量缩减和所有权审计也只扫描对应世界，避免复制和
  扫描全服登记。`status` 同时显示活跃陷阱与活跃 owner 数；
...
- 小队依次经历 `FORMING → BRIEFING → DEPLOYING → ENGAGING`。会议时成员沿原版可达路径站到首领
  ...
  默认 8 格紧急圈时会直接交战，不会为了“演完动画”挨打；Paper 心跳直接把四个已校验计时标量传入
  共享阶段规划器，不逐队构造临时计时快照；
- 撤退、骷髅紧急脱离和苦力怕引信继续拥有更高优先级；蜘蛛会议期间暂停个人跳扑，进入交战后才恢复
  预测扑击。蜘蛛的载客门控直接使用 `Entity#isEmpty()`，不会为布尔判断构造乘客转换列表。通用小队 Goal
  只在非交战阶段持有 `MOVE/LOOK`，不会每 tick 传送或改坐标；
- 协调器每 `5 tick` 在主线程通过共享 `BoundedSpatialIndex` 更新实体所在桶。索引以对象身份登记并按
  世界 UUID 隔离；移动/跨世界时删除旧桶后 O(1) 登记新桶，配置重载则按新半径完整重建。桶坐标由
  无依赖的 primitive-long 开放寻址表保存，桶内成员由身份双向节点保持稳定顺序；九宫格查询不创建
  临时坐标键或集合迭代器。协调器的全局成员顺序也由 `MemberRecord` 双向链维护，刷新和失效清理直接
  沿节点进行。默认单次最多检查 64 个原始候选、接收 20 人，因此不存在全服成员两两互扫；
- 同一成员的武器、盾牌、远程、载具和通用命令 Goal 会在一个 tick 内多次读取命令。协调器缓存不可变
  `PaperSquadDirective`；同 tick 直接命中，跨 tick 时若全部可见字段、焦点和目的地仍精确相同也复用旧快照，
  任一值变化才替换。每队复用一个几何缓存槽，焦点、首领位置、目标方向与首领到目标方向全部按
  tick/revision 覆写为 primitive 坐标并传给阵位规划器，只有最终阵位需要物化；指令的焦点兼容视图仅在诊断调用时
  惰性创建。首领实体对象替换会提升修订号使所有成员同步失效。缓存未命中时直接读取实体 XYZ/yaw，远程射界的线段投影和最近点也改用
...
一次 240 tick 掩体周期、失败后的 60 tick 搜索冷却和完整重试；若终检已经观察到探头射击但回撤计数
尚未落地，会再有界轮询最多 60 tick，不会把合法动作尾帧或重规划误报为失败。第四条相隔 160 格的
远距通道生成独行烟花弩手和 500 点生命铁傀儡，
要求副手火箭真实消耗、集中弹体服务至少发射并碰撞引爆一次，清理后活跃弹体数回到零。若 420 tick
终检正好遇到在途弹体，自测会先把夹具射手 IQ 降到 1 停止新装填，再最多轮询 60 tick 等服务收敛。
...
探针始终要求生产 Goal 至少进入一次 wind-up；若 JFR 等采样负载下专属 owner 到 160 tick 仍未落网，
才调用同一个 `PaperWebTrapService` 在隔离中点补放，并在结果中公开 `webTrapFallbackUsed`，避免把调度
时序抖动误报成 owner 索引故障。该断言不会依赖白天蜘蛛是否随机放弃某个独立目标。
混编样本中的苦力怕爆炸半径临时设为 0、引信延长到 200 tick，因此可以观察载客行为而不破坏测试世界。
战斗主窗口结束时，仍存活的掩体探针若尚未完成首箭或回撤，会获得最多 60 tick 的统一有界收尾窗口，超时仍严格失败。无论成功、失败、重载还是插件关闭，测试实体
