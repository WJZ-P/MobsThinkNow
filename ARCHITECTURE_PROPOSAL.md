...
│     ├─ SquadDangerMemory/Kind           32 条限容、带 TTL 的共享危险位置黑板
│     ├─ SquadThreatMemory/Allocator      8 条可见威胁证据与至少 60% 主目标分配
│     ├─ SquadBlastReservationBook        苦力怕预测爆点短租约、冲突与强制提交
│     ├─ SquadFiringLaneRegistry          骷髅八段弹道短租约与挡线查询
│     ├─ SquadFiringLaneClearGoal         挡线成员左右两点可达侧移
│     ├─ SquadAssaultGeometry            交叉射界、侧后爆破和移动火力阵位纯数学
│     ├─ SquadPreparationGoal            溺尸/苦力怕/蜘蛛/巨人集结与部署命令适配
│     ├─ SquadMemberHeartbeat            无近战包装成员的 O(1) 感知心跳
│     ├─ SquadMeetingFormation           职位感知的多排半圆会议席位纯几何
│     ├─ SquadBriefingRoutePlanner       原阵位/有序后备路线到真实报告的常数时间选择层
│     ├─ SquadRouteOutcome               可达、改派、阻塞三态路线报告
│     ├─ SquadSocialChoreography          路线报告/IQ/装备画像到动作与注视场景的纯函数
...
├─ command/OverworldAssaultShowcaseSpawner 八成员联合兵种编队生成事务
...
- 两次确认的路线失败进入 `SquadDangerMemory`。每队最多 32 条记录，路线检查最多均匀采样 16 个节点；
  会议、部署、改令和射界让位共享这张黑板，条目到期或小队解散后自然清空。
- `SquadThreatMemory` 只接收真实受击事件和当前 LOS 目标，容量固定为 8 并逐 tick 衰减；分配器最多
  选两个次要目标和 40% 合格成员，首领、载具关系与已提交爆破手不参与转派。若外部 Goal 临时改写
  未获批准的目标，下个决策周期会把该成员恢复到分配目标，保证 60% 主攻比例不是只读诊断值。
...
- 同期 `SquadFiringLaneRegistry` 保存五 tick 的八段弧线租约；队友只对与自己 AABB 相交的走廊让位，
  每次最多验证左右两个候选。射手开火、失去 LOS、进入会议或停止 Goal 即释放，过期条目兜底清理；
...
- `SquadBlastReservationBook` 以苦力怕 ID 保存八 tick 预测爆点租约；相同目标或重叠危险区只允许一个
  普通引信提交，候补者停在危险圈外侧翼。续约移动也重新判冲突；外部点燃可强制登记，但下一 tick
  会让仍可撤销的普通预约退火。队友撤离比较当前实体位置和预测爆心，选择相对危险度更高的一处；
...
- 普通蜘蛛完成出生流程时进行一次常数时间特质抽取：`4%` 获得永久速度 I、`1%` 获得永久速度 II，
  药水粒子保持可见；已有同级或更强速度效果时不覆盖，洞穴蜘蛛不参与；
- `SpiderCreeperCarrierGoal` 优先级 2，高于个人跳扑和近战。相同目标的双方先进入 `ASSEMBLING`，
  2.8 格内由苦力怕执行至少 3 tick、最多 9 tick 的抛物线登乘跳，蜘蛛停步接应；进入接取范围后
  才通过 `startRiding` 建立真实乘客树，再进入 `DELIVERING` 与 `FINAL_CHARGE`。载荷登乘时立即朝向
  共享目标，之后每 tick 更新 LookControl；实体调度造成的
...
- 原版 `Mob#getControllingPassenger` 会把第一只 Mob 乘客视为驾驶者并暂停坐骑 MOVE/LOOK。
  ...
  委托 `super`；
- 每次组合先抽取配置上限的 `88%～100%` 作为个体上限，再由双方较高 IQ 和难度从 `1.10` 插值；
  默认配置上限 `1.40`、可配硬上限 `1.70`。进入苦力怕自身起爆
...
转换、关服或读档时恢复真正的双手装备。
进入 `TNT_PLACE` 时技能状态机同时持有持续动作 `ENGINEER_WORK`；客户端根据临时工具实际所在手
镜像单膝操作姿势。装药被点燃、落点失效或 Goal 被抢占时，只按动作所有者精确释放该状态，
不会清掉同时发生的更高优先级撤退动作；真实放置与点燃挥手仍在各自命中帧覆盖准备姿势。

## 2.7 下界派系战斗边界

- `PiglinMixin` 与 `PiglinBruteMixin` 在原版 `Brain.tick + updateActivity` 完成后调用
  `PiglinBattleLineController`。控制器不创建仇恨、不扫描同伴，只从原版 `ATTACK_TARGET` 读取当前
  合法目标，再按实体 ID 奇偶与模 3 结果生成稳定射击/近战通道。最终候选必须具备坚固地基、完整
  碰撞净空与可到达 `Path`，然后只覆盖 `WALK_TARGET` 和 `LOOK_TARGET`；每 16～27 tick 决策一次；
- `BlazeMixin` 保留原版 priority 4 攻击 Goal，并在 priority 3 安装
  `SmartBlazeAttackGoal`。配置开启时它取得 MOVE/LOOK，执行偏好射程、固定盘旋方向、近身撤离、
  24～36 tick 难度化充能与 2～4 发弹幕；关闭时 `canUse()` 立即为 false，原版 Goal 无缝接管。
  `BlazeChargeAccess` 只桥接原版私有 charged 标志，不复制同步数据；
- `GhastMixin` 追加 priority 0 的 16 格垂直玩家索敌，原版 4 格 Goal 继续作为关闭配置时的回退。
  `GhastShootFireballGoalMixin` 只在功能开启时取消原 `tick`，保留 10 tick 声音、20 tick 发射帧、
  大火球实体和爆炸威力；速度提前量最多 12 tick，每次发射后用 `MoveControl` 请求 22～28 格外圈
  炮位，不直接传送；
- `HoglinMixin` 与 `ZoglinMixin` 复用 `HoglinChargeController`，但目标仍来自各自原版 Brain。
  `IDLE → WINDUP → CHARGING → RECOVERING` 在 6～13 格、垂直差不超过两格且视线成立时启动；
  蓄力末再次沿直线每两格检查地基/净空，随后只施加一次冲量并以 `lastHurtMobTimestamp` 避免与
...
- `FORMING`：确认成员和首领，生成集结点；僵尸首领抬臂招手发出第一次召集；
- `RALLYING`：成员向首领附近聚拢；若集结超过 20 tick，首领每 36 tick 才补一次招手，不逐 tick 刷新；
- `BRIEFING`：成员站成面向战场的多排半圆；默认 64 tick 内完成巡视、商议、左右翼真实路线报告、
  必要的改派命令和最终确认。全队注视目标在发言者、被点名成员、阵位和敌人之间切换；
- `DEPLOYING`：首领挥臂前指发布总攻，成员按两 tick 间隔确认后按任务从集结点散开；
- `ENGAGING`：正面位交给原版近战 Goal，侧翼和截断位继续执行目的地；
- `REORGANIZING`：首领丢失后从存活成员中立即选出继任者；默认 48 tick 内完成成员环顾、
  新首领举手/举武器接任、成员点头和恢复指挥四段换届仪式。
...
职位解决“单个成员负责什么”，`SquadAssaultPlanner` 进一步解决“整支队伍怎样同时进攻”。每次组建、
换届或成员变化时，协调器用一次 O(K) 阵容快照和首领智力选择下表方案，并把结果冻结在当前小队，
不会每 tick 随机改主意：

| 总攻方案 | 解锁条件 | 联合行为 |
|---|---|---|
| `SWARM` | 兜底，或首领 IQ `1～4` | 保留职位规则并直接压上 |
| `PIN_AND_FLANK` | IQ `5+`、近战与远程齐备 | 远程正面牵制，近战从两翼切入 |
| `SHIELD_WEDGE` | IQ `6+`、盾卫与远程/爆破齐备 | 盾卫居中前压，后排获得准备窗口 |
| `CROSSFIRE` | IQ `7+`、至少两名射手与近战 | 射手按稳定序号交替站到目标左右射界 |
| `MOUNTED_BREACH` | IQ `7+`、蜘蛛与苦力怕齐备 | 载具先到侧后 staging point，再提交最终引信 |
| `COMBINED_ARMS` | IQ `8+`、僵尸/骷髅/苦力怕/蜘蛛齐备 | 正面盾楔、双侧火力、侧后爆破与移动射手同时工作 |

运输分配继续是一对一确定结果。`DEPLOYING` 时，被蜘蛛预约的乘员保持原地，载具主动接取；
`ENGAGING` 时，苦力怕载具在 50 tick 内尝试抵达侧后阵位，目标进入 3.25 格、路径失败或超时便
降级为普通冲锋。骷髅载具按其 IQ 调整后的偏好射程留在交叉射界，蜘蛛只移动、不替乘员结算射击；
弓/弩 Goal 仍负责真实瞄准、装填和弹体。所有阵位只是原版导航目的地，地形不可达时不会传送。
...
7. 路径按决策间隔更新，目的地未明显变化时复用现有 Path。会议寻路只在进入 `BRIEFING` 或
   首领换届时执行：最多检查左/右翼各一名，每名至多“原路线 + 3 个有序后备”共 4 条 Path，
   不在会议的 64 个 tick 内重复扫描，也不随小队人数做两两比较。
...
15. 巨人运输逻辑不执行范围实体搜索。协调器重建职位时只对单队至多 K 名成员排序一次，并通过三阶段
    ...
    精确实体 ID 预约。抛投弹道是常数时间向量数学。近战只在“选择新动作”和“唯一命中帧”各执行
    一次半径 `7.6` 的局部索引查询，前摇/后摇的其余 tick 不扫描实体，不会退化成巨人之间的 N²。
...
| `netherAiEnabled` | `true` | 第一批猪灵、烈焰人、恶魂、疣猪兽与岩浆怪战术总开关 |
| `piglinFormationTactics` | `true` | 猪灵弩手散列射击通道与近战/蛮兵分线 |
| `blazeCombatTactics` | `true` | 烈焰人盘旋、拉距、充能与难度化弹幕 |
| `blazePreferredRange` | `10.0` | 烈焰人偏好射程，范围 `7～16` 格 |
| `netherPredictionStrength` | `0.70` | 烈焰弹与恶魂火球速度提前量，范围 `0～1` |
| `ghastArtilleryTactics` | `true` | 恶魂 16 格垂直索敌、预测炮击与射后换位 |
| `hoglinChargeTactics` | `true` | 成年疣猪兽/僵尸疣猪兽的安全直线蓄力冲锋 |
| `hoglinChargeSpeed` | `1.15` | 冲锋寻路倍率与有限起步冲量，范围 `0.85～1.45` |
...
| `briefingTicks` | `64` | 战术会议持续时间，范围 `60～100` tick |
| `deploymentTimeoutTicks` | `80` | 部署最长时间 |
| `regroupTicks` | `48` | 首领换届仪式与重组时间，范围 `40～80` tick |
...
| `swordFeints` | `true` | 高智力剑士是否对正在格挡的目标使用无伤佯攻 |
| `swordFeintMinimumIntelligence` | `7` | 掌握佯攻所需最低智力，范围 `1～10` |
| `swordFeintChance` | `0.35` | 每个合格攻击轮次选择佯攻的概率，范围 `0～1` |
| `shieldBashes` | `true` | 高智力盾卫是否可把一次成功格挡转换为盾击 |
| `shieldBashMinimumIntelligence` | `7` | 掌握盾击所需最低智力，范围 `1～10` |
| `shieldBashChance` | `0.35` | 每次真实成功格挡选择盾击的概率，范围 `0～1` |
| `shieldBashDamage` | `2.0` | 盾击唯一命中帧伤害，范围 `0～8` |
| `shieldBashKnockback` | `1.25` | 盾击水平击退强度，范围 `0～3` |
...
- `SquadSocialChoreography` 是不访问世界和实体的纯场景层：`FORMING` 召集、长集结补召、
  64 tick 会议、48 tick 换届和部署回应只由 `state + squadId + phase + participant snapshots` 决定。
  快照包含原会议职位、真实路线报告、智力、稳定 UUID 键和装备画像；`squadId` 只用于稳定轮换发言者，
  多支小队不会动作完全同步；
- 会议开始前，协调器用实体真实 `Navigation#createPath` 检查左/右翼原阵位，再按缩短侧翼、截断、
  施压的固定顺序寻找第一条可达后备。`CLEAR` 产生点头，`REROUTED/BLOCKED` 产生摇头与纠正命令；
  同一报告会覆写部署职位/目的地并递增 `planEpoch`，因此表现不是与决策脱节的固定演出；
- `Scene.Attention` 同时给出普通成员和首领的语义注视对象：发言实体、指定职位的真实部署点或目标。
  `SquadDirective.focusPosition` 在服务端解析成眼部/阵位坐标，准备 Goal 与僵尸控制器平滑旋转身体；
- 等待集结时只在 UUID 决定的稀疏单 tick 播放职业小动作。盾卫敲盾、剑士检刃、斧手扛斧、工程兵
  检查工具，低 IQ 空手成员歪头；客户端按两手真实占用选择空手或武器手，双持时保留另一只装备手；
- 换届场景按成员稳定顺序先环顾，再由新首领敬礼接管，成员错峰点头，最后重发命令。声音只落在
  召集、命令、异议和接任等关键拍，纯观察/商议/点头只用少量粒子，避免 20 只小队形成连续噪声；
- `SquadTheatrics` 只在节拍命中的 tick 写一次动作 ID 和世界开始时间；客户端
  `ZombieBodyAnimation` 本地采样招手、巡视、左右指令、点头、摇头、商议和前指总攻的全身关键帧，
  不同步逐 tick 骨骼。撤退、武器前摇等更高优先级动作可拒绝社交动作，指标只统计真正发布成功的
  `leaderSocialGestures` / `memberSocialGestures`；
...
- **智能武器战斗**：`ZombieWeaponCombat` 独立于装备发放开关，作用于所有普通
  ...
  冷却结束后剑手正常重新近身；智力达到门槛且目标正在格挡时，每轮只抽一次佯攻概率，
  假挥阶段不调用 `doHurtTarget`，第 6 tick 后目标放盾才交回真实攻击，完整 18 tick 仍举盾则
  后撤并短恢复；24 tick 重试门控避免逐 tick 重抽。斧手在水平距离 1.8～3.3 格、垂直差不高于
  1.25 格且跳跃包围盒无碰撞时，先停步执行 8 tick 举斧前摇，结束时重新验证几何才起跳；
  空中仅做轻微航向修正，只有下落速度低于
...
- **盾卫**：持械僵尸按 `armedShieldChance` 额外获得副手盾（持矛者除外，
  ...
  武器冷却完成后才放盾。高智力个体会按配置概率把成功格挡转换成 14 tick 盾击，
  第 5 tick 对 3 格内有视线目标执行唯一一次伤害与击退；`BASHING` 同时属于持位和攻击阻断状态，
  因而副手盾撞与主手挥击不会在同 tick 双重结算。未触发时打开最多 10 tick 的单次攻击窗口；观察时限自然结束时也可
...
- JUnit：效用选择、配置边界、六类总攻方案门槛、可见战术证据积分/迟滞/过期、阵容约束下的自适应总攻、
  ...
  盾卫随机观察、反击窗口与盾击资格边界、身体动作 ID/优先级/时效、社交编排的召集防刷屏、
  64 tick 真实路线回应/智力延迟/语义注视/48 tick 换届/部署错峰，以及半圆站位间距、路线后备顺序、
  装备感知手势、职业待机、继任环顾/敬礼和前指总攻关键帧的轴向、镜像与收势、
...
- Minecraft 服务端 GameTest：真实 Mob 上的活动租约同级互斥、高优先级抢占、旧所有者续租失败、精确释放与 3 tick 过期；
  ...
  接任命令、部署总攻与错峰确认，并校验首领/成员成功动作指标；受击撤退触发与 5 格安全半径终止、
  盾卫无攻击时随机试探/成功格挡后延迟 2～4 tick 放盾反击/挥击后延迟重举，以及盾击在
  第 5 tick 唯一造成配置伤害和远离方向速度冲量、剑士假挥阶段零伤害且目标放盾后才放行真攻击、
  斧手同步 8 tick 举斧前摇、空中动作与真实下落跳劈 1.5 倍伤害、
  ...
  入口逐条验证十二种僵尸家族子命令；工程兵另覆盖 TNT 放置/点燃全过程单膝动作、点燃后释放，
  真实放置/点燃 TNT、owner 与 80 tick
  ...
  苦力怕成为最高智力首领，并验证高 IQ 四物种队伍选择 `COMBINED_ARMS`；联合兵种命令另从真实
  Brigadier 入口验证每组恰好生成三名僵尸职业、两名骷髅火力、一只苦力怕和两只蜘蛛，以及两组
  批量事务的十六个根实体；巨人另覆盖生产 Mixin Goal、重型属性、头顶/双手三挂点、乘员不夺控制、
  ...
  额外乘员；阵型间距按本批最大真实碰撞宽度自适应，末影人另覆盖生产 Mixin Goal/职业分配
   ...
   六个复数分类、`nether`/`overworld_assault` 分类与五十七个战术/变种 literal 全量注册，并真实批量生成十四类基础实体；本轮另验证
   爆点排队、预测爆心疏散、射击通道让位与五人队 60/40 威胁分配；当前共 152 项
  服务端 GameTest；
